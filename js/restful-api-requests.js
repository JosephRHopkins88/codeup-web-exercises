...
const getBooks = () => {
    fetch("https://cactus-languid-erica.glitch.me/books")
        .then(resp => resp.json())
        .then(books => {
            console.log(books);
            let htmlStr = "";
...
fetch("https://cactus-languid-erica.glitch.me/books/1")
    .then(resp => resp.json())
        ...
        .then(books => {
            for (let book of books) {
                if(book.title !== newBook.title || book.author.firstName !== newBook.author.firstName || book.author.lastName !== newBook.author.lastName) {
                    fetch("https://cactus-languid-erica.glitch.me/books", postOptions)
                        .then(getBooks);
                } else {
...
fetch("https://cactus-languid-erica.glitch.me/books/6", putOptions).then(getBooks);
...
fetch("https://cactus-languid-erica.glitch.me/books/7", patchOptions).then(getBooks);
...
$("#uniqueBooks").click(() => {
    ...
    fetch(`https://cactus-languid-erica.glitch.me/books/${inputVal}`, deleteOptions)
        .then(getBooks);
})
