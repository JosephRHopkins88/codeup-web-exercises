    ...
    $.get("https://api.openweathermap.org/data/2.5/onecall?units=imperial&lat=" + coordinates[0] + "&lon=" + coordinates[1] + "&exclude=current,hourly,minutely&appid=" + WEATHER_MAP_TOKEN)
        .done(function (resp) {
            ...
            for (var i = 0; i < 5; i++) {
                ...
                document.getElementById("temp" + i).innerHTML = "Temperature:<br/>Morning:" + " " + resp.daily[i].temp.morn.toFixed(0) + "&deg;F" + "<br/>Day:" + " " + resp.daily[i].temp.day.toFixed(0) + "&deg;F" + "<br/>Evening:" + " " + resp.daily[i].temp.eve.toFixed(0) + "&deg;F" + "<br/>Night:" + " " + resp.daily[i].temp.night.toFixed(0) + "&deg;F" + "<br/>Min:" + " " + resp.daily[i].temp.min.toFixed(0) + "&deg;F" + "<br/>Max:" + " " + resp.daily[i].temp.max.toFixed(0) + "&deg;F";
...
function generateAllHtml(resp) {
    var myHtml = "";

    for (var i = 0; i < 5; i++) {
        myHtml += "<div className='card col-2'><div className='card-body'><h4 id='todayDate" + i + "' className='card-title'>" + new Date(resp.daily[i].dt * 1000).toDateString() + "</h4><div id='icon" + i + "' className='card-text'><img src='http://openweathermap.org/img/wn/" + resp.daily[i].weather[0].icon + "@2x.png' alt='The weather icon'/></div><hr><div id='temp" + i + "' className='card-text'>Temperature:<br/>Morning: " + resp.daily[i].temp.morn.toFixed(0) + "&deg;F<br/>Day: " + resp.daily[i].temp.day.toFixed(0) + "&deg;F<br/>Evening: " + resp.daily[i].temp.eve.toFixed(0) + "&deg;F<br/>Night: " + resp.daily[i].temp.night.toFixed(0) + "&deg;F<br/>Min: " + resp.daily[i].temp.min.toFixed(0) + "&deg;F<br/>Max: " + resp.daily[i].temp.max.toFixed(0) + "&deg;F</div><div id='wind_speed" + i + "' className='card-text'>Wind speed: " + resp.daily[i].wind_speed + "</div><div id='sunrise" + i + "' className='card-text'>Sunrise: " + new Date(resp.daily[i].sunrise * 1000).toLocaleTimeString() + "</div><div id='sunset" + i + "' className='card-text'>Sunset: " + new Date(resp.daily[i].sunset * 1000).toLocaleTimeString() + "</div></div></div>";
    }
    document.getElementById("weather_row2").innerHTML = myHtml;
document.getElementById("todayDate0").className = "card-title";
    if (resp.daily.length > 0) {
        document.getElementById("weather_row2").style.display = "flex";
    }
}